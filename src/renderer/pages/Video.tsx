...
export default function Video() {
  ...
  const [selectedGalleries, setSelectedGalleries] = React.useState<Set<string>>(
    new Set(),
  );
  ...
  React.useEffect(() => {
    if (video.isSuccess) {
      setSelectedTags(
        new Set(video.data.tags.map((tag: ITagModel) => tag.title)),
      );
      setSelectedGalleries(
        new Set(
          video.data.galleries.map(
            (gallery: IGalleryModel) => gallery.galleryPath,
          ),
        ),
      );
    }
  ...
  return (
    <VideoView
      ...
      handleGenerateTgp={
        generateTgp as UseMutateFunction<unknown, unknown, string, unknown>
      }
      allItems={{
