...
export default function CheckBoxGroups({
  ...
  const [prevSelectedItems, setPrevSelectedItems] = React.useState<Set<string>[]>([])
  ...
  const handleSelectAll = (listIndex: number, checked: boolean): void => {
    const newSet: Set<string> = checked ? new Set(lists[listIndex].allItems) : new Set()
...
const getDiffObj = (prevItems: Set<string>, curItems: Set<string>): IDiffObj => {
