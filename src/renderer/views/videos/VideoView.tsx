...
const VideoView = ({
  ...
  updateVideoRelations: UseMutateFunction<
    string,
    unknown,
  ...
  <>
    ...
    {activeTab === 'tgp' &&
      (video.isTgpAvailable ? (
        ...
        <SpinnerOr isSpinner={isGeneratingTgp} msg="Generating TGP...">
          <Button
            appearance="primary"
            onClick={async () => await handleGenerateTgp(video.id)}
          >
            Generate TGP
          </Button>
        </SpinnerOr>
    ...
    {activeTab === 'relations' && (
      <CheckBoxGroups
        ...
        saveHandlers={[
          async (ts) => setSelectedItems.tags(ts),
          async (gs) => setSelectedItems.galleries(gs),
        ]}
